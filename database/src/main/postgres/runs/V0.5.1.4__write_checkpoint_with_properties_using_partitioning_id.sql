...
--      i_checkpoint_properties     - optional checkpoint properties as key-value pairs
--
-- Returns:
--      status              - Status code
...
BEGIN
  ...
  -- Insert checkpoint properties if provided
  IF i_checkpoint_properties IS NOT NULL THEN
    INSERT INTO runs.checkpoint_properties (
      fk_checkpoint,
      property_name,
      property_value
    )
    SELECT
      ...
      key,
      value
    FROM each(i_checkpoint_properties);
  END IF;
  ...
  FROM (SELECT unnest(i_measurements) AS measurement) UN;
...
ALTER FUNCTION runs.write_checkpoint(BIGINT, UUID, TEXT, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, JSONB[], BOOLEAN, TEXT, HSTORE) OWNER TO atum_owner;
GRANT EXECUTE ON FUNCTION runs.write_checkpoint(BIGINT, UUID, TEXT, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, JSONB[], BOOLEAN, TEXT, HSTORE) TO atum_user;
