...
BEGIN
  ...
  FROM runs.write_checkpoint(
    ...
    i_by_user,
    i_checkpoint_properties
  ) WC INTO status, status_text;
...
ALTER FUNCTION runs.write_checkpoint(JSONB, UUID, TEXT, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, JSONB[], BOOLEAN, TEXT, HSTORE) OWNER TO atum_owner;
GRANT EXECUTE ON FUNCTION runs.write_checkpoint(JSONB, UUID, TEXT, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, JSONB[], BOOLEAN, TEXT, HSTORE) TO atum_user;
