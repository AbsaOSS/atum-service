...
class MockAtumAgentImplAndUnitTest extends AtumAgent {
  ...
  override def measurePublish(measure: Measurement): Unit = measure match {
    ...
    case SumOfHashesOfColumn(MockMeasureNames.hashSumOfNames, _, result) =>
      assert(result.contains("2044144307532"))

    case SumOfValuesOfColumn(MockMeasureNames.sumOfValuesOfSalary, _, result) =>
      assert(result.contains("2988144"))
