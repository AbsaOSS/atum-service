...
//    val actualAdditionalDataDTO =
    decode[AdditionalDataSubmitDTO](additionalDataDTOJson) match {
      case Left(_) => fail()
      case Right(value) => assert(value == expectedAdditionalDataDTO)
    }

//    assert(Right(expectedAdditionalDataDTO))
  }
...
//         |}
//         |""".linearize
//    val actualCheckpointDTOJson = checkpointDTO.asJson(checkpointDTO)
//
