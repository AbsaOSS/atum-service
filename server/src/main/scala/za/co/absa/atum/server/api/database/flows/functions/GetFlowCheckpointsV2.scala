...
import za.co.absa.atum.model.dto.CheckpointQueryDTOV2
...
class GetFlowCheckpointsV2 (implicit schema: DBSchema, dbEngine: DoobieEngine[Task])
  extends DoobieMultipleResultFunctionWithAggStatus[CheckpointQueryDTOV2, CheckpointFromDB, Task](values =>
    Seq(
      fr"${values.iPartitioningId}",
      fr"${values.limit}",
