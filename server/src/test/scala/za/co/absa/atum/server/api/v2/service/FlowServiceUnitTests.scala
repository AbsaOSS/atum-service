...
object FlowServiceUnitTests extends ZIOSpecDefault with TestData {
  ...
  when(flowRepositoryMock.getFlowCheckpoints(1L, 1, 1L, None, None, includeProperties = false))
    .thenReturn(ZIO.succeed(ResultHasMore(Seq(checkpointWithPartitioningDTO1))))
  when(flowRepositoryMock.getFlowCheckpoints(2L, 1, 1L, None, None, includeProperties = false))
    ...
    suite("FlowServiceSuite")(
      suite("GetFlowCheckpointsV2Suite")(
        test("Returns expected PaginatedResult[CheckpointV2DTO]") {
          for {
            result <- FlowService.getFlowCheckpoints(1L, 1, 1L, None, None, includeProperties = false)
          } yield assertTrue {
            result == ResultHasMore(Seq(checkpointWithPartitioningDTO1))
          }
        },
        test("Returns expected ServiceError") {
          assertZIO(FlowService.getFlowCheckpoints(2L, 1, 1L, None, None, includeProperties = false).exit)(
