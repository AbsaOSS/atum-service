...
object CheckpointServiceUnitTests extends ZIOSpecDefault with TestData {
  ...
  when(checkpointRepositoryMock.getPartitioningCheckpoints(1L, 10, 0L, None, None, None, None, None, includeProperties = false))
    .thenReturn(ZIO.succeed(ResultHasMore(Seq(checkpointV2DTO1))))
    when(checkpointRepositoryMock.getPartitioningCheckpoints(0L, 10, 0L, None, None, None, None, None, includeProperties = false))
      .thenReturn(ZIO.fail(NotFoundDatabaseError("Partitioning not found")))
    ...
    suite("CheckpointServiceSuite")(
      ...
      suite("GetPartitioningCheckpointsSuite")(
        test("Returns expected Right with Seq[CheckpointDTO]") {
          for {
            result <- CheckpointService.getPartitioningCheckpoints(1L, 10, 0L, None, None, None, None, None, includeProperties = false)
          } yield assertTrue {
            result == ResultHasMore(Seq(checkpointV2DTO1))
          }
        },
        test("Returns expected NotFoundServiceError") {
          assertZIO(CheckpointService.getPartitioningCheckpoints(0L, 10, 0L, None, None, None, None, None, includeProperties = false).exit)(
            failsWithA[NotFoundServiceError]
          )
        }
      )
