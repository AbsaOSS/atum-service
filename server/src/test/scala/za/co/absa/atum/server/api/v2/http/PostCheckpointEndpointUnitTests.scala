...
object PostCheckpointEndpointUnitTests extends ZIOSpecDefault with TestData {

  private val checkpointControllerMock = mock(classOf[CheckpointController])

  when(checkpointControllerMock.postCheckpoint(1L, checkpointV2DTO1))
    .thenReturn(ZIO.succeed((SingleSuccessResponse(checkpointV2DTO1, uuid1), "some location")))
  when(checkpointControllerMock.postCheckpoint(1L, checkpointV2DTO2))
    .thenReturn(ZIO.fail(GeneralErrorResponse("error")))
  when(checkpointControllerMock.postCheckpoint(1L, checkpointV2DTO3))
    .thenReturn(ZIO.fail(InternalServerErrorResponse("error")))
  when(checkpointControllerMock.postCheckpoint(1L, checkpointV2DTO4))
    .thenReturn(ZIO.fail(ConflictErrorResponse("error")))
  when(checkpointControllerMock.postCheckpoint(0L, checkpointV2DTO4))
  ...
  private val postCheckpointServerEndpointV2 = Endpoints.postCheckpointEndpoint
