...
object CheckpointRepositoryUnitTests extends ZIOSpecDefault with TestData {
  ...
  when(getPartitioningCheckpointsMock.apply(GetPartitioningCheckpointsArgs(0L, 10, 0L, None, None, None)))
    .thenReturn(ZIO.right(Seq(Row(FunctionStatus(11, "ok"), Some(checkpointItemPaginatedFromDB)))))
  when(getPartitioningCheckpointsMock.apply(GetPartitioningCheckpointsArgs(1L, 10, 0L, None, None, None)))
    .thenReturn(ZIO.right(Seq(Row(FunctionStatus(11, "ok"), None))))
  when(getPartitioningCheckpointsMock.apply(GetPartitioningCheckpointsArgs(3L, 10, 0L, None, None, None)))
    .thenReturn(ZIO.left(DataNotFoundException(FunctionStatus(41, "Partitioning not found"))))
    ...
    suite("CheckpointRepositorySuite")(
      ...
      suite("GetPartitioningCheckpointsSuite")(
        test("Returns expected Seq without properties") {
          for {
            result <- CheckpointRepository.getPartitioningCheckpoints(0L, 10, 0L, None, None, None, includeProperties = false)
          } yield assertTrue(
        ...
        test("Returns expected Seq with properties") {
          for {
            result <- CheckpointRepository.getPartitioningCheckpoints(0L, 10, 0L, None, None, None, includeProperties = true)
          } yield assertTrue(
        ...
        test("Returns expected Seq.empty") {
          for {
            result <- CheckpointRepository.getPartitioningCheckpoints(1L, 10, 0L, None, None, None, includeProperties = false)
          } yield assertTrue(
        ...
        test("Returns expected NotFoundDatabaseError") {
          assertZIO(CheckpointRepository.getPartitioningCheckpoints(3L, 10, 0L, None, None, None, includeProperties = false).exit)(
            failsWithA[NotFoundDatabaseError]
          )
        }
      )
