...
object PatchPartitioningParentV2IntegrationTests extends ConfigProviderTest {
  override def spec: Spec[TestEnvironment with Scope, Any] = {
    suite("PatchPartitioningParentSuite")(
      test("Returns expected PatchParentV2DTO with provided partitioning") {
        val partitioningId: Long = 999L
        val parentPartitioningId: Long = 1111L
        val byUser: String = "Jack"
        for {
          patchPartitioningParent <- ZIO.service[PatchPartitioningParent]
          result <- patchPartitioningParent(PatchPartitioningParentArgs(partitioningId, parentPartitioningId, byUser))
        } yield assertTrue(result == Left(DataNotFoundException(FunctionStatus(41, "Child Partitioning not found"))))
      }
    ).provide(
      PatchPartitioningParent.layer,
